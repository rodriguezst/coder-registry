...
variable "ai_provider" {
  type        = string
  description = "The AI provider to use (anthropic, openai, google)."
  default     = "anthropic"
  validation {
    condition     = contains(["anthropic", "openai", "google"], var.ai_provider)
    error_message = "AI provider must be one of: anthropic, openai, google"
  }
}

locals {
  # we have to trim the slash because otherwise coder exp mcp will
  # set up an invalid opencode config 
...
resource "coder_script" "opencode" {
  ...
  script       = <<-EOT
    ...
    if [ "${var.install_opencode}" = "true" ]; then
    ...
    module_path="$HOME/.opencode-module"
    mkdir -p "$module_path/scripts"

    # save the prompt for the agentapi start command
    echo -n "$CODER_MCP_OPENCODE_TASK_PROMPT" > "$module_path/prompt.txt"
    ...
    if [ "${var.experiment_report_tasks}" = "true" ]; then
      echo "Configuring OpenCode to report tasks via Coder MCP..."
      export CODER_MCP_APP_STATUS_SLUG="${local.opencode_app_slug}"
      export CODER_MCP_AI_AGENTAPI_URL="http://localhost:3284"
      coder exp mcp configure opencode "${local.workdir}"
    fi
    ...
    if ! command_exists opencode; then
      echo "Error: OpenCode is not installed. Please enable install_opencode or install it manually."
      exit 1
    fi
...
resource "coder_ai_task" "opencode" {
  sidebar_app {
    id = coder_app.opencode_web.id
  }
}
...
# if the first argument is not empty, start opencode with the prompt
if [ -n "$1" ]; then
  cp "$module_path/prompt.txt" /tmp/opencode-prompt
else
...
function start_agentapi() {
  ...
  local prompt_subshell='"$(cat /tmp/opencode-prompt)"'
  ...
  agentapi server --term-width 67 --term-height 1190 -- \
    bash -c "opencode $continue_flag $prompt_subshell" \
    > "$log_file_path" 2>&1
}

echo "Starting AgentAPI..."

# attempt to start opencode with the --continue flag
...
# if there was no conversation to continue, opencode exited with an error.
# start opencode without the --continue flag.
if grep -q "No conversation found to continue" "$log_file_path"; then
  echo "AgentAPI with --continue flag failed, starting opencode without it."
  start_agentapi
  exit_code=$?
fi
